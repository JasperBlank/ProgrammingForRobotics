...
public:
   ...
   // concatenate list in argument to the current list
   void concatenate(List<NODETYPE>& inputList) {
      // set the first pointer of the input list to the current pointer, which connects both lists
      ListNode<NODETYPE>* current = inputList.firstPtr;

      while (current != nullptr) { // while the end of the list is not reached
         insertAtBack(current->data); // access the data of the node and insert it to the current list
         current = current->nextPtr; // updates the current pointer to the next node
      }
      
      // to empty the argument list, it is possible to set both first and last pointers as null pointers
